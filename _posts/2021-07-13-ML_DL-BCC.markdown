...
- BCC model의 learning task는 parameter인 $$\alpha_{1}, \alpha_{2}, \Theta$$을 likelihood of observing matrix X을 최대화 하는 값을 추론 하는 것이다.
...
$$q(z_{1},z_{2},\pi_{1},\pi_{2} \lvert \gamma_{1}, \gamma_{2}, \phi_{1},\phi_{2}) = \Pi_{u=1}^{n_{1}} q(\pi_{1u} \lvert \gamma_{1u}) \Pi_{v=1}^{n_{2}} q(\pi_{2v} \lvert \gamma_{2v}) \Pi_{u=1}^{n_{1}} \Pi_{v=1}^{n_{2}} q(z_{1uv} \lvert \phi_{1u})  q(z_{2uv} \lvert \phi_{2v}) $$
...
$$ \log p(X \lvert \alpha_{1}, \alpha_{2}, \Theta) \geq E_{q} (\log p(X,z_{1},z_{2},\pi_{1},\pi_{2} \lvert \alpha_{1}, \alpha_{2}, \Theta)) - E_{q} (\log q(X,z_{1},z_{2},\pi_{1},\pi_{2} \lvert \gamma_{1}, \gamma_{2}, \phi_{1},\phi_{2})) $$
...
- Inference step에서는 $$ \log p(X \lvert \alpha_{1},\alpha_{2},\Theta)$$의 **lower bound (L)를 $$(\alpha_{1},\alpha_{2},\Theta)$$를 설정하여 Maximize한다.**
