...
- Linearly Nonseparable Case (Kernel function)
...
- c 는 margin과 traning-error에 대한 trade-off를 결정하는 hyper parameter
...
$$subject \; to \quad y_{i}(w^{T}x_{i} + b ) \geq 1 -\xi_{i} \; y \subset {1,2,3,...n} $$   
...
위의 조건식은 다음과 같이 쓸수 있다.  
$$ \un
$$ where \quad\sum_{i=1}^{n}\alpha_{i}y_{i} = 0 \quad C-\alpha_{i} - \gamma_{i} = 0 $$
